...
KNOB<BOOL>KnobPinPlayReplayer(KNOB_MODE_WRITEONCE, KNOB_FAMILY, KNOB_REPLAY_NAME, "0", "Replay a pinball");
KNOB<BOOL>KnobPinPlayLogger(KNOB_MODE_WRITEONCE,  KNOB_FAMILY, KNOB_LOG_NAME, "0", "Create a pinball");
...
//==============================================================================
KNOB<string> KnobSiNUCAOutputFile(KNOB_MODE_WRITEONCE, "pintool",
...
struct data_instr{
    char const *rtn_name;
    char const *instr_name;
    UINT32 instr_len;
};

//==============================================================================
void hmc_set_data_instr(data_instr *hmc_data, char const *rtn_name, char const *instr_name, UINT32 instr_len){
    hmc_data->rtn_name = rtn_name;
    hmc_data->instr_name = instr_name;
    hmc_data->instr_len = instr_len;
};

//==============================================================================
VOID hmc_trace_instruction(RTN hmc_rtn, data_instr *hmc_data){
    opcode_package_t NewInstruction;
    char bbl_count_str[TRACE_LINE_SIZE];
    char opcode_str[TRACE_LINE_SIZE];
    uint32_t a = 0, b = 0, c = 0, i, j, k;
    std::vector<int> rregs(32), wregs(32), biregs(32), deps(32);
    int ireg, breg;
    std::string rtn_name;
    ...
    if (RTN_Valid(hmc_rtn)) {
        ...
        RTN_InsertCall(hmc_rtn, IPOINT_BEFORE, (AFUNPTR)hmc_write_memory, IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_UINT32, hmc_data->instr_len, IARG_UINT32, count_trace, IARG_THREAD_ID, IARG_END);
        RTN_InsertCall(hmc_rtn, IPOINT_BEFORE, (AFUNPTR)write_dynamic_char, IARG_PTR, hmc_bbl_str, IARG_THREAD_ID, IARG_END);

        // int conta = 0;
        // Identify read/write registers
        for(INS ins = RTN_InsHead(hmc_rtn); INS_Valid(ins); ins = INS_Next(ins)){
            // conta++;
            for (i = 0; i < MAX_REGISTERS; i++) {
                wregs[i] = POSITION_FAIL;
                rregs[i] = POSITION_FAIL;
            }
            for (i = 0; i < INS_MaxNumRRegs(ins); i++) {
                rregs[i] = INS_RegR(ins, i);
            }
            for (j = 0; j < INS_MaxNumWRegs(ins); j++) {
                wregs[j] = INS_RegW(ins, j);
            }
            ...
            if(a == 0){
                for(; a < i; a++){
                    NewInstruction.read_regs[a] = rregs[a];
                }
            }
            else{
                for(k = 0; k < rregs.size(); k++){
                    for(i = 0; i < a; i++){
                        if(rregs[k] == NewInstruction.read_regs[i] || rregs[k] == 0)
                            break;
                        if(i == a - 1){
                            for(j = 0; j < b; j++){
                                if(rregs[k] == NewInstruction.write_regs[j])
                                    break;
                                if(j == b - 1)
                                    NewInstruction.read_regs[a++] = rregs[k];
                            }
                        }
                    }
                }
            ...
            if(b == 0){
                for(; b < j; b++){
                    NewInstruction.write_regs[b] = wregs[b];
                }
            }
            else{
                for(k = 0; k < wregs.size(); k++){
                    for(j = 0; j < b; j++){
                        if(wregs[k] == NewInstruction.write_regs[j] || wregs[k] == 0)
                            break;
                        if(j == b - 1)
                        NewInstruction.write_regs[b++] = wregs[k];
                    }
                }
            }
            for(k = 0; k < wregs.size(); k++){
            ...
            if(breg != 0){
                  for(i = 0; i < a; i++){
                      if(breg == NewInstruction.read_regs[i])
                          break;
                      if(i == a - 1)
                          biregs[c++] = breg;
                  }
            ...
            if(ireg != 0){
                for(i = 0; i <= a; i++){
                    if(ireg == NewInstruction.read_regs[i])
                        break;
                    if(i == a - 1)
                        biregs[c++] = ireg;
                }
        ...
        if (!biregs.empty())
            for (i = 0; i < c; i++){
                NewInstruction.read_regs[++a] = biregs[i];
            }
        NewInstruction.opcode_address = RTN_Address(hmc_rtn);
        strcpy(NewInstruction.opcode_assembly, hmc_data->instr_name);
        NewInstruction.opcode_size = hmc_data->instr_len;
        if((rtn_name.compare(4, 17, "_hmc128_saddimm_s") == 0) ||
...
void x86_to_static(const INS& ins) {
    ...
    switch (iclass) {
        ...
        // FP SQRT 3D NOW
        case XED_ICLASS_PFSQRT:         // Square root
        // FP SQRT SSE
...
VOID ImageLoad(IMG img, VOID *) {
    ...
    hmc_set_data_instr(&hmc_data[3], "_hmc128_bswap_s", "HMC_BITSWAP_SINGLE_128OPER", 16);
    hmc_set_data_instr(&hmc_data[4], "_hmc128_and_s", "HMC_AND_SINGLE_128OPER", 16);
    hmc_set_data_instr(&hmc_data[5], "_hmc128_nand_s", "HMC_NAND_SINGLE_128OPER", 16);
    hmc_set_data_instr(&hmc_data[6], "_hmc128_nor_s", "HMC_NOR_SINGLE_128OPER", 16);
    hmc_set_data_instr(&hmc_data[7], "_hmc128_or_s", "HMC_OR_SINGLE_128OPER", 16);
    hmc_set_data_instr(&hmc_data[8], "_hmc128_xor_s", "HMC_XOR_SINGLE_128OPER", 16);
    hmc_set_data_instr(&hmc_data[9], "_hmc64_cmpswapgt_s", "HMC_CMPSWAPGT_SINGLE_64OPER", 8);
    hmc_set_data_instr(&hmc_data[10], "_hmc64_cmpswaplt_s", "HMC_CMPSWAPLT_SINGLE_64OPER", 8);
    hmc_set_data_instr(&hmc_data[11], "_hmc128_cmpswapz_s", "HMC_CMPSWAPZ_SINGLE_128OPER", 16);
    hmc_set_data_instr(&hmc_data[12], "_hmc128_cmpswapgt_s", "HMC_CMPSWAPGT_SINGLE_128OPER", 16);
    hmc_set_data_instr(&hmc_data[13], "_hmc128_cmpswaplt_s", "HMC_CMPSWAPLT_SINGLE_128OPER", 16);
    ...
    // ~
                ...
                for (uint32_t i = 0; i < OMP_barrier_master_end.size() && !found_GOMP; i++){
                    ...
                    if (strcmp(rtn_name.c_str(), OMP_barrier_master_end[i]) == 0) {
                        printf("rotina: %s  OMP: %s\n", rtn_name.c_str(), OMP_barrier_master_end[i]);
                        RTN_Open(rtn);
...
int main(int argc, char *argv[]) {
    ...
    SINUCA_PRINTF("Inserted Output File Name = %s\n", KnobSiNUCAOutputFile.Value().c_str());

    sprintf(stat_file_name , "%s.tid0.stat.out.gz", KnobSiNUCAOutputFile.Value().c_str());
    ...
    for (UINT32 i = 0; i < max_threads; i++){
        dyn_file_name[0] = '\0';
        sprintf(dyn_file_name, "%s.tid%d.dyn.out.gz", KnobSiNUCAOutputFile.Value().c_str(), i);
    ...
    for (UINT32 i = 0; i < max_threads; i++){
        mem_file_name[0] = '\0';
        sprintf(mem_file_name, "%s.tid%d.mem.out.gz", KnobSiNUCAOutputFile.Value().c_str(), i);
