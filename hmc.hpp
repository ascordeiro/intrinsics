...
typedef uint64_t __h64l2[2]; // __attribute__((vector_size(16))); // 2 long integer
...
// 8-byte Bit Write
void _hmc64_bwrite_s(__h64l1 *mem_op, __h64l1 bit_mask, __h64l1 write_data){
    *mem_op = *mem_op & (bit_mask & write_data);
}
...
__h64l1 _hmc64_cmpswapeq_s(__h64l1 *mem_op, __h64l1 imm_op){
    __h64l1 ret = *mem_op;
    if (*mem_op == imm_op)
        *mem_op = imm_op;
    return ret;
...
//8-byte Equal tos
__h64l1 _hmc64_equalto_s(__h64l1 mem_op, __h64l1 imm_op){
    if (mem_op == imm_op)
        return 1;
    return 0;
}
...
//16-byte Equal to
__h128ll1 _hmc128_equalto_s(__h128ll1 mem_op, __h128ll1 imm_op){
    if (mem_op == imm_op)
        return 1;
    return 0;
}
